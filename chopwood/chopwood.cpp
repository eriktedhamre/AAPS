...
int main() {
    ...
    // Keep count if a number is left in tree
    ...
    while(u.size() < cuts && it != v.end()){
        ...
        // Reduce the frequency of the connecting node in the map by one
        // until it has zero frequency and then add it to available numbers
        int v_value = *it;
    ...
    if(u.size() < cuts || upper_limit == 0){ printf("Error\n");}

    for(int i = 0; i < u.size(); i++) { printf("%d\n", u[i]);}
}
